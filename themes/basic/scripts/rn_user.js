...
(function ($, Drupal, drupalSettings) {
    ...
    attach: function (context, settings) {
      ...
      if ($passwordInput.length) {
        ...
        $passwordInputParentWrapper
          ...
          .parent()
          .append('<div aria-live="polite" aria-atomic="true" class="password-confirm js-password-confirm"><span class="title">' + translate.confirmTitle + '</span> <span class="result"></span></div>')
        ...
        var $confirmChild = $confirmResult.find('span.result');
        ...
        var passwordCheckMatch = function (confirmInputVal) {
          ...
          $confirmChild.html(translate['confirm' + (success ? 'Success' : 'Failure')])
            .parent().removeClass('ok error').addClass(confirmClass);
          $confirmInput.removeClass('ok error').addClass(confirmClass);
        };
        ...
        var passwordCheck = function () {
          if (settings.password.showStrengthIndicator) {
            ...
            $passwordInputParent.find('.js-password-strength__text').html(result.indicatorText)
              .parent()
              .removeClass('is-weak is-fair is-good is-strong')
              .addClass(result.indicatorTextClass);
          }
  ...
  Drupal.evaluatePasswordStrength = function (password, translate) {
    ...
    if (strength < 60) {
      ...
      indicatorTextClass = 'is-weak';
      indicatorClass = 'is-weak';
    }
    else if (strength < 70) {
      indicatorText = translate.fair;
      indicatorTextClass = 'is-fair';
      indicatorClass = 'is-fair';
    }
    else if (strength < 80) {
      indicatorText = translate.good;
      indicatorTextClass = 'is-good';
      indicatorClass = 'is-good';
    }
    else if (strength <= 100) {
      indicatorText = translate.strong;
      indicatorTextClass = 'is-strong';
    ...
    return {
      ...
      indicatorTextClass: indicatorTextClass,
