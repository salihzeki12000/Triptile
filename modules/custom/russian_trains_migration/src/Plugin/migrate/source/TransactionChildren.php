...
/**
 * Source plugin for the TransactionChildren.
 *
 * @MigrateSource(
 *   id = "transaction_children"
 * )
 */
class TransactionChildren extends SqlBase {
...
//      ->condition('ypt.updated_at', '2017-07-10 04:10:20', '>');
    $query->fields('yppt', ['id', 'payment_transaction_id', 'parent_transaction_id', 'ack', 'created_at', 'updated_at']);
    ...
    $query->addField('yppct', 'payment_transaction_id', 'parent_id');
    ...
    $query->join('ya_payment_paypal_transaction', 'yppct', 'yppct.transaction_id=yppt.parent_transaction_id');
    $query->join('sf_guard_user', 'sgu', 'sgu.id=yo.user_id');
    $query->condition('ypt.merchant_id', ['PaypalRT', 'PaypalAU'], 'IN');
    $query->isNotNull('yppt.parent_transaction_id');
