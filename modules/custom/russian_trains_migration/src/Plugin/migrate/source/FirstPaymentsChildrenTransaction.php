...
/**
 * Source plugin for the FirstPaymentsChildrenTransaction.
 *
 * @MigrateSource(
 *   id = "first_payments_children_transaction"
 * )
 */
class FirstPaymentsChildrenTransaction extends SqlBase {
  ...
  public function query() {
    $query = $this->select('ya_payment_firstpayments_transaction', 'ypft');
...
//      ->condition('ypt.updated_at', '2017-07-10 04:10:20', '>');
    $query->fields('ypft', ['id', 'payment_transaction_id', 'parent_transaction_id', 'created_at', 'updated_at']);
    ...
    $query->addField('ypfct', 'payment_transaction_id', 'parent_id');
    $query->fields('sgu', ['email_address']);
    $query->join('ya_payment_transaction', 'ypt', 'ypt.transaction_id=ypft.payment_transaction_id');
    ...
    $query->join('ya_payment_firstpayments_transaction', 'ypfct', 'ypfct.transaction_id=ypft.parent_transaction_id');
    $query->join('sf_guard_user', 'sgu', 'sgu.id=yo.user_id');
    $query->condition('ypt.merchant_id', 'FirstPayments');
    $query->isNotNull('ypft.parent_transaction_id');
