...
namespace Drupal\it_train_provider\Form;
...
class LogForm extends FormBase {
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    $files = file_scan_directory('public://it_train_provider', '/.*/');
    ksort($files);
    $options = [];
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    foreach ($form_state->getValue('files') as $filename => $delete) {
      if ($delete) {
        file_unmanaged_delete('public://it_train_provider/' . $filename);
        $count++;
      }
    }
